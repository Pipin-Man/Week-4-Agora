...
export const GET: APIRoute = async (context) => {
  ...
  if (!items.length) {
    return new Response('<li class="px-3 py-3 text-xs text-zinc-500 dark:text-zinc-400">No unread notifications</li>', {
  ...
  const html = items
    .map((item) => {
      ...
      return `<li class="border-b border-zinc-200/70 px-3 py-3 last:border-b-0 dark:border-zinc-800/70">
          ...
          <div class="flex items-start gap-2 p-2">
            ...
            <div class="min-w-0">
              <div class="flex items-center gap-2 text-xs">
                ...
                <time class="text-zinc-500 dark:text-zinc-400">${when}</time>
              </div>
              <p class="mt-0.5 line-clamp-3 whitespace-pre-wrap break-words text-sm text-zinc-800 dark:text-zinc-200">${body}</p>
              <p class="mt-0.5 text-[11px] text-zinc-500 dark:text-zinc-400">#${roomName}</p>
            </div>
          </div>
        </a>
      </li>`;
